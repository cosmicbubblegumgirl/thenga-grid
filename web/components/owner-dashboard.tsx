  ...
  return (
    <div className="owner-layout">
      ...
      <section className="owner-content">
        ...
        {section === 'orders' && <section className="order-board">{['reserved', 'packing', 'ready'].map((status) => <div className={`order-lane ${status}`} key={status}><h2><i />{status === 'reserved' ? 'New' : status}<b>{data.orders.filter((order) => order.status === status).length}</b></h2>{data.orders.filter((order) => order.status === status).map((order) => <article key={order.id}>{order.paymentStatus === 'settled' && <div className="owner-payment"><span>PAID · OPEN PAYMENTS</span><strong>R{(Number(order.paidAmountValue ?? order.priceCents) / 100).toFixed(2)} received</strong></div>}<div><strong>{order.customerName}</strong><small>{order.productName}</small></div><span>R{(order.priceCents / 100).toFixed(2)}</span><p>Pickup code <b>{order.pickupCode}</b></p>{status !== 'ready' ? <Button onClick={() => void updateOrder(order.id, status === 'reserved' ? 'packing' : 'ready')}>{status === 'reserved' ? 'Start packing' : 'Mark ready'}</Button> : <Button onClick={() => void updateOrder(order.id, 'collected')}>Collected</Button>}</article>)}</div>)}</section>}
